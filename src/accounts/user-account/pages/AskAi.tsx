  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (account) {
        ...
        const aiResult = await web3.ora.calculateAIResult(
          account,
          Models.LLAMA2,
          PROMPT,
          fee.toString()
        );
        const response: string = await web3.ora.getAIResult(
          Models.LLAMA2,
          PROMPT
        );
...
function MessageCard({ message }: { message: string }) {
  return <div className="p-5 bg-white rounded-lg">{message}</div>;
}
